...
parser.add_argument('-s', '--string_end', default='testtest', help='The postfix of the results filename.', type=str)
parser.add_argument('-t', '--test', help='Which dataset to test on.', type=str, default='cpp',
...
parser.add_argument('-b', '--balance_samples', default=3, type=int, choices=[0, 1, 2, 3],
...
extra_time = 10000
frame_data_cpp = ut.read_frame_data(base_path_cpp, 0)

salsa_cpp_keys = list(frame_data_cpp.keys())
print(len(salsa_cpp_keys))
train_set_keys = salsa_cpp_keys[128:]
test_set_keys = salsa_cpp_keys[:64]

train_dict = dict((k, frame_data_cpp[k]) for k in train_set_keys)
test_dict = dict((k, frame_data_cpp[k]) for k in test_set_keys)
...
for frame_id, frame_info in train_dict.items():
    ...
    for group in frame_info:
        if len(group) == 1:
            ...
            group_id_tracker += 1
        for person in group:
            data = ut.fetch_person_data(person, float(frame_id), base_path_cpp)
            pos_x = float(data[0])
...
for _ in range(fold_count):
    ...
    models.append([model, pred, optimizer])
pos_edge_count = 0
neg_edge_count = 0

val_length = math.floor(len(train_set)/fold_count)
# print('VAL LENGTH', val_length)

for run in range(fold_count):
    model = models[run][0]
    pred = models[run][1]
    optimizer = models[run][2]
    fold_val_set = set(train_set[run*val_length:(run+1)*val_length])
    fold_tra_set = list(set(train_set).difference(fold_val_set))
    # print('SET lengths', len(train_set), len(fold_val_set), len(fold_tra_set))
    for single_train_graph in fold_tra_set:
        ...
        for e in range(epochs):
            ...
            loss.backward()
            optimizer.step()
    done_models.append([model, pred, optimizer])
...
for single_val_idx, single_val_graph in enumerate(test_set):
    ...
    with torch.no_grad():
        ...
        for fold in range(fold_count):
            model = done_models[fold][0]
            pred = done_models[fold][1]
            ...
            # print('TEST LABELSt', test_labels)
            fold_test_labels.append(test_labels)
        ensembled_labels = []
        for col in zip(*fold_test_labels):
            # print((sum(col)/fold_count).item(), type((sum(col)/fold_count).item()))
            ensembled_labels.append(int(round((sum(col)/fold_count).item())))
            # print(sum(col.item)/fold_count)
        # ens_labels = [(sum(col.item)/fold_count) for col in zip(*fold_test_labels)]
        # ensembled_labels = [round(num) for num in ens_labels]
        # print('ENSEMBLED', ensembled_labels)
        # exit()

        to_remove = []
        for i in range(len(ensembled_labels)):
            if ensembled_labels[i] == 0:
                to_remove.append(i)
